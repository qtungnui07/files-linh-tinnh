...
dataset_path = input()
confirmed_images_file = "confirmed.txt"  # File lưu ảnh không được chọn
...
def load_images_and_labels():
    ...
    for filename in os.listdir(dataset_path):
        if filename.endswith(".jpeg"):
            ...
            if len(images_and_labels) >= 70:  # Chỉ load tối đa 40 ảnh
                break

    return images_and_labels

# Hàm tính tiến độ công việc
def calculate_progress():
    # Số dòng trong confirmed.txt
    confirmed_images = load_confirmed_images()
    num_confirmed = len(confirmed_images)
    
    # Tổng số ảnh trong folder dataset
    total_images = sum(1 for filename in os.listdir(dataset_path) if filename.endswith(".jpeg"))
    
    # Tính phần trăm
    if total_images == 0:
        return 0
    return (num_confirmed / total_images) * 100
